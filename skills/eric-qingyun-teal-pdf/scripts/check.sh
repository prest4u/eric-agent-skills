#!/usr/bin/env bash
# 青云识别册 skill 自检：可发现、4 页、无禁印、无旧 D 残留、typst 可编译。
set -euo pipefail
...
echo "== eric-qingyun-teal-pdf self-check =="
...
if [[ ! -f "$SKILL" ]]; then
  ...
  if [[ "$name" == "eric-qingyun-teal-pdf" && -n "$desc" && ${#desc} -ge 20 ]]; then
...
# theme leftovers from old D
if [[ ! -f "$THEME" ]]; then
  fail "theme.typ missing"
else
  HIT_THEME=0
  for s in "mark-square" "rail-head" "#F4F6F5"; do
    if grep -F -- "$s" "$THEME" >/dev/null 2>&1; then
      fail "theme.typ still contains old-D leftover: $s"
      HIT_THEME=1
    fi
  done
  if [[ "$HIT_THEME" -eq 0 ]]; then
    pass "theme.typ has no mark-square / rail-head / #F4F6F5"
  fi
fi
...
if ! command -v typst >/dev/null 2>&1; then
  ...
  TMPDIR_C="$(mktemp -d -t qingyun-teal)"
...
# banned brand + old chrome labels
if [[ -f "$PDF" ]]; then
  TEXT=""
