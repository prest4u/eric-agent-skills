#!/usr/bin/env python3
"""Self-check for the 青云新闻栏 skill: chassis I tokens, pages, banned words."""
...
REQUIRED_THEME_SNIPPETS = (
    'rgb("#D2CBB8")',
    'rgb("#111111")',
    '("Heiti SC", "Hiragino Sans GB", "PingFang SC")',
    '("Songti SC", "STSong")',
    "top: 15mm",
    "bottom: 11mm",
    "left: 11mm",
...
def check_theme() -> None:
    ...
    if missing:
        raise SystemExit("theme.typ missing I tokens: " + ", ".join(missing))
    leaked = [s for s in FORBIDDEN_THEME_SNIPPETS if s in text]
    if leaked:
        raise SystemExit("theme.typ leaked E/D/C chrome: " + ", ".join(leaked))
    if DOCUMENT.is_file():
...
def main() -> int:
    ...
    print(
        ...
        f"theme_tokens=I | paper=#D2CBB8 | banned_brands=none"
    )
    return 0
