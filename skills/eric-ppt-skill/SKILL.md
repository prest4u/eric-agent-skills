---
name: eric-ppt-skill
description: 【演示文稿】Create, edit, replicate, read, and export presentations. For every PPT task, the default deliverables are BOTH (1) a self-contained PPTD project folder containing the .pptd manifest plus pages/media dependencies and (2) a locally generated .pptx with fade slide transitions and font embedding when the selected export path supports it. Use for any presentation, PowerPoint, PPT/PPTX, slide deck, PPTD, infographic, or poster task unless the user explicitly requests another format. Deliver with normal local file/folder links using absolute paths.
---

# Definition
Eric PPT Skill is a presentation creation and export workflow built around the PPTD format and a browser-side PPTX writer. It defines a YAML-format intermediate DSL (`.pptd`) that abstracts OOXML and keeps each page self-contained.
...
2. the matching locally generated `.pptx`, with fade slide transitions applied by default and font embedding enabled when the browser export path supports it.
...
Default delivery includes PPTX export and optional browser-based visual QA, which need a local toolchain. **Before generating**, verify:

1. **python3**: run `python3 --version` (on Windows, `python` may be the correct command). It runs the portable OOXML exporter and all validators.
2. **PyYAML 6.0.3**: required to parse PPTD. Dependencies are never installed automatically; if missing, the script prints the pinned installation command and stops. Ask before installing it.
3. **Default PPTX export is browser-free and offline**. The included Python exporter writes editable native PowerPoint objects and supports the common PPTD surface: rich text, preset shapes, lines, local PNG/JPEG/GIF images, editable table cells, and an editable chart fallback. Download remote media into `media/` before export.
4. **Optional visual QA / browser export** requires a separately licensed compatible editor supplied through `ERIC_PPT_EDITOR`, plus **Node.js 18+**, **agent-browser 0.33.2**, **Pillow 11.1.0**, and **websocket-client 1.8.0**. The public Skill intentionally does not bundle any proprietary product frontend, patched WASM, browser binary, font, or template.
...
2. Visual review with rendered page images is **required when an authorized renderer is already available and the model supports image input (multimodal)**. Use one of these reviewed paths:
   - Preferred portable path: export the PPTX, render it with an already installed office suite, inspect all page images, then edit PPTD and re-export until it passes.
   - Optional PPTD-editor path: set `ERIC_PPT_EDITOR` to a separately licensed editor, then run `scripts/export_images.py`. It loads the deck into that local editor, exports images, and stitches an overview. It needs Chromium via `agent-browser`; the public Skill does not include the editor or browser.
...
4. PPTX conversion: use `scripts/export_pptx.py`. **Default path is the included portable Python OOXML writer** (`scripts/portable_ooxml.py`): no cookie, signature API, browser UI, or remote runtime dependency. Optional `--browser` uses only a separately licensed editor explicitly configured with `ERIC_PPT_EDITOR`.
5. Default PPTX options:
   - page transition: `fade` (淡入淡出), written to every slide after export;
   - font embedding: not performed by the portable path; it keeps font-family declarations and uses native editable objects. A separately licensed browser editor may provide embedding;
   - override with `--transition none`; use `--browser` only when `ERIC_PPT_EDITOR` points to an editor the user is authorized to use.
6. Export command:

   ```bash
   # preferred: portable Python OOXML (offline)
   python3 ./scripts/export_pptx.py \
     /abs/path/project/deck.pptd \
     --output /abs/path/project/deck.pptx

   # optional: separately licensed local browser editor
   ERIC_PPT_EDITOR=/path/to/authorized-editor \
   python3 ./scripts/export_pptx.py \
     /abs/path/project/deck.pptd \
...
7. Portable and optional-editor model:
   ...
   - **Manual edit / preview**: open the generated `.pptx` in PowerPoint, Keynote, LibreOffice, or another authorized office application. A compatible PPTD browser editor is an optional user-supplied integration.
   - Missing optional dependencies produce safe, pinned installation instructions; scripts never run npm/pip installation themselves.
   - Local PNG/JPEG/GIF files inside the PPTD project are embedded by the portable exporter. Convert SVG to PNG first. Remote/data URLs are rejected until downloaded locally, which keeps the runtime deterministic and the deck self-contained.
...
9. When the user wants manual browser editing, use locally available editor assets only. Ask the user to authorize the complete PPTD project directory. Use a Chromium-based browser for writable access; folder-upload fallback is read-only. Remote images or fonts referenced by the deck may still be fetched from their hosts.
10. After completing and delivering any presentation, end with a concise optional next step explaining that the editable PPTD project can be reopened for further revisions and exported again. Keep this reminder in addition to, not instead of, the required project and file links.
