...
def exam_stem_text(value: object) -> str:
    """Render MCQ/cloze stem slots separately from workbook writing blanks."""
    ...
    punctuation = r"[.,;:!?，。；：！？、]?"
    return re.sub(r"(?P<blank>_{3,})(?P<punct>" + punctuation + r")", repl, esc(value))
...
def page_in_profile(meta: dict, profile: str, book: dict) -> bool:
    ...
    if audience in {"teacher", "teacher-only"} or template == "teacher-answer-key":
...
def sentence_map_cards(headers: list[str], rows: list[list[str]]) -> str:
    labels = headers or ["Sentence", "Analysis", "Check"]
    labels = (labels + ["Sentence", "Analysis", "Check"])[:3]
    cards = []
    for idx, row in enumerate(rows, 1):
        cells = (list(row) + ["", "", ""])[:3]
        fields = "".join(
            f"""
            <div class="sentence-map-card-field sentence-map-card-{slugify(labels[i], f'field-{i+1}')}">
              <b>{esc(labels[i])}</b>
              <p>{inline_text(cells[i], blank_mode='compact')}</p>
            </div>
            """
            for i in range(3)
        )
        cards.append(f"<article><span class=\"sentence-map-index\">{idx:02d}</span><div>{fields}</div></article>")
    return (
        '<section class="sentence-map-card-stack" data-component="categorizing-chart" '
        'data-surface-family="sentence-map" data-surface="a4-card-stack">'
        + "".join(cards)
        + "</section>"
    )
...
def page_shell(num: int, meta: dict, book: dict, body: str, profile: str) -> str:
    ...
    if template not in {"cover", "title", "unit-opener"}:
        ...
        if section == "frontmatter":
            footer_label = f"{esc(meta.get('folio_label', 'BOOK MAP'))} • {esc(book.get('title', 'Student Book'))}"
        footer = f"""
        <footer class="folio">
          <b>{num}</b>
...
def render_sentence_map(meta: dict, book: dict, assets: dict) -> str:
    ...
    <main class="body-page sentence-map-page">
      ...
      {diagram_callout(meta.get('diagram', {}))}
      {sentence_map_cards(meta.get('chart', {}).get('headers', []), meta.get('chart', {}).get('rows', []))}
...
def css(book: dict, tokens: dict, profile: str) -> str:
    ...
    * {{ box-sizing: border-box; }}
    ...
    .sentence-map-card-stack article {{
      ...
      gap: 10pt;
      padding: 9pt 11pt;
      border: 0.8pt solid #cfddda;
      background:
        linear-gradient(90deg, rgba(19,126,148,.065), transparent 96pt),
    ...
    .sentence-map-card-field {{
      display: grid;
      grid-template-columns: 76pt 1fr;
      gap: 8pt;
      padding: 4pt 0;
      border-bottom: 0.55pt solid rgba(65,121,126,.18);
    }}
    .sentence-map-card-field:last-child {{ border-bottom: 0; }}
    .sentence-map-card-field b {{
      color: var(--teal-dark);
      font-size: 8.3pt;
      text-transform: uppercase;
      letter-spacing: .02em;
    ...
    .guided-mcq-set p .exam-stem-slot {{
      ...
      min-width: 42pt;
      height: .68em;
      border-bottom: .82pt solid #2f4d50;
      vertical-align: -0.12em;
      margin: 0 2.4pt;
    }}
    .guided-mcq-set p .exam-stem-slot-short {{ min-width: 34pt; }}
    .guided-mcq-set p .exam-stem-slot-wide {{ min-width: 58pt; }}
    .guided-mcq-set p .exam-stem-keep {{
      white-space: nowrap;
      display: inline-block;
    }}
...
<head>
  ...
  <style>{css(book, tokens, profile)}</style>
</head>
