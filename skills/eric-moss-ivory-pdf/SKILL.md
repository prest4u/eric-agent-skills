...
Create quiet, warm editorial PDFs. Preserve the official two-accent hierarchy while adapting page roles to the content.
...
- Use the exact display name `Eric Moss Ivory PDF｜苔绿象牙`.
- Keep the paper warm ivory, never bright yellow or beige.
- Use moss green as the primary structural accent and slate blue as the secondary accent.
- Target an approximate 70:30 moss-to-slate emphasis ratio.
- Moss carries Chinese structure, primary rules, and selected quotations. Slate carries English eyebrows, secondary labels, and analytical cues.
- Never let both accents compete inside one small block. Preserve grayscale readability.
...
When a structural change is intended for both official themes, render the same bounded sample with `$eric-slate-white-pdf`. The page roles, line breaks, hierarchy, and spacing should remain valid when only theme tokens change. Fix the shared structure before adding theme-specific exceptions.
