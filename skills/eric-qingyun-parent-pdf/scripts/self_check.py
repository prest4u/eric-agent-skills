#!/usr/bin/env python3
"""Self-check for the 青云家长说明册 skill: tokens, pages, banned brand words."""
...
FORBIDDEN_THEME_SNIPPETS = (
    ...
    'rgb("#F3F3F1")',  # C near-white
    "#5D6C75",         # A primary rail
    "#626A56",         # B moss
)
...
def check_theme() -> None:
    ...
    if missing:
        raise SystemExit("theme.typ missing parent-handbook tokens: " + ", ".join(missing))
    leaked = [s for s in FORBIDDEN_THEME_SNIPPETS if s in text]
    if leaked:
        raise SystemExit("theme.typ leaked A/B/C chrome: " + ", ".join(leaked))
...
def check_pdf(pdf: Path, expect_pages: int) -> int:
    ...
    if "院校冲" in extracted or "冲稳保表" in extracted:
        raise SystemExit("Looks like a volunteer 冲稳保 table, not a 家长说明册")
...
def main() -> int:
    ...
    print(
        ...
        f"theme_tokens=parent-handbook | banned_brands=none"
    )
    return 0
