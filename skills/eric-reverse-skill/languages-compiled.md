...
.NET 托管程序（C# 编译的 exe/dll）先从 CLR metadata 与 IL 入手；只有 IL 无法解释关键行为时，再下沉到 native 层。
...
- 脱混淆：ConfuserEx / SmartAssembly / Babel / .NET Reactor → 先识别壳与保护类型，再选择 `de4dot` 或对应工具
- **IL 优先**：关键判断与 patch 用 IL 编辑器而非 C# 反编译器（避免编译器生成代码重编译失真）
- 对 Sharp* 工具先恢复参数、网络端点、序列化格式与关键调用链；不要因为家族名相似就直接套用结论

**边界：** IL2CPP / NativeAOT 编译产物是 native（无 CLR），不走 dnSpy；改用 IDA、Ghidra 或 radare2。Codegate 2013 两阶段 XOR+AES-CBC 模式见 [tools.md](tools.md#net-analysis) 的 `.NET Analysis` 段。
