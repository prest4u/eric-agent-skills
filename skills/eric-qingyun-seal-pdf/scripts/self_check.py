#!/usr/bin/env python3
"""Self-check for the 青云 seal/sand-scroll skill: tokens, pages, banned brand words."""
...
DEFAULT_PDF = ROOT / "samples" / "选科指导报告-小样.pdf"
...
def check_theme() -> None:
    ...
    if missing:
        raise SystemExit("theme.typ missing seal/sand tokens: " + ", ".join(missing))
    leaked = [s for s in FORBIDDEN_THEME_SNIPPETS if s in text]
    if leaked:
        raise SystemExit("theme.typ leaked A/B/parent/near-white chrome: " + ", ".join(leaked))
...
def main() -> int:
    ...
    print(
        ...
        f"theme_tokens=seal-sand | banned_brands=none"
    )
    return 0
